...
int main(int argc, char *argv[]) {
  ...
  RT_CHECK(vx_spatial_mem_alloc(device, buf_size, VX_MEM_READ, &A_buffer, size, size, size, size / 2, size / 2, size));
  RT_CHECK(vx_mem_address(A_buffer, &kernel_arg.A_addr));
  RT_CHECK(vx_spatial_mem_alloc(device, buf_size, VX_MEM_WRITE, &B_buffer, size, size, size, size / 2, size / 2, size));

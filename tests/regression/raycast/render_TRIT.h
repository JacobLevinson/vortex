...
// helpers for raw bit moves between float and uint32
  ...
  while (stackPtr != 0) {
    ...
    if (node.isLeaf()) {
      ...
      for (uint32_t i = 0; i < node.triCount; ++i) {
         ...
         *  Hardware path
         * ------------------------------------------------------------------ */
        // 1.   Broadcast the ray to the CSRs that the TRIT instruction uses
        csr_write(VX_CSR_TRIT_RO1, f2u(ray.orig.x));
        csr_write(VX_CSR_TRIT_RO2, f2u(ray.orig.y));
        csr_write(VX_CSR_TRIT_RO3, f2u(ray.orig.z));
        csr_write(VX_CSR_TRIT_RD1, f2u(ray.dir.x));
        csr_write(VX_CSR_TRIT_RD2, f2u(ray.dir.y));
        csr_write(VX_CSR_TRIT_RD3, f2u(ray.dir.z));

        // 2.   Launch the custom instruction – pass *word address* of triangle
        const tri_t *triPtr = &triBuffer[triIdx];
        float dist = u2f(vx_trit(reinterpret_cast<uintptr_t>(triPtr)));

        // 3.   Gather barycentrics back out of CSRs
        float3_t bcoords;
        bcoords.x = u2f(csr_read(VX_CSR_TRIT_B1)); // w1
        bcoords.y = u2f(csr_read(VX_CSR_TRIT_B2)); // w2
        bcoords.z = u2f(csr_read(VX_CSR_TRIT_B3)); // w3 = 1‑w1‑w2

        // 4.   Same hit test as before
        if (dist < hit->dist) {
          hit->dist = dist;
          hit->bcoords = bcoords;
          hit->blasIdx = blasIdx;

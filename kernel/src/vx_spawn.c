...
//   // If this thread does not have valid tasks, return early
...
static void __attribute__((noinline)) process_thread_groups_spatial() {
  ...
  uint32_t warps_per_group = targs->warps_per_group;
  uint32_t groups_at_once = targs->groups_at_once; // = warps_per_core / warps_per_group
  uint32_t group_size = targs->group_size;

  /* local warp‑group inside the core */
  uint32_t local_group_id = warp_id / warps_per_group;
  uint32_t group_warp_id = warp_id - local_group_id * warps_per_group; // same as warp_id % warps_per_group

  /* thread inside the group */
  uint32_t local_task_id = group_warp_id * threads_per_warp + thread_id;
  if (local_task_id >= group_size) {
    return; // inactive threads leave early
  }

  /* CUDA‑style threadIdx */
  threadIdx.x = local_task_id % blockDim.x;
  threadIdx.y = (local_task_id / blockDim.x) % blockDim.y;
  threadIdx.z = local_task_id / (blockDim.x * blockDim.y);

  /* ---- ROUND‑ROBIN BLOCK SCHEDULE ---- */

  uint32_t start_x = targs->start_block_x + local_group_id; // different x‑offset per warp‑group
  uint32_t end_x = targs->end_block_x;
  uint32_t stride = groups_at_once; // round‑robin distance

  uint32_t start_y = targs->start_block_y;
  uint32_t end_y = targs->end_block_y;
  uint32_t total_z = gridDim.z;

  vx_kernel_func_cb callback = targs->callback;
  const void *arg = targs->arg;

  /*  y‑major → z‑major → x‑round‑robin  */
  for (uint32_t block_z = 0; block_z < gridDim.z; ++block_z) {
    for (uint32_t block_y = targs->start_block_y; block_y < targs->end_block_y; ++block_y) {
      for (uint32_t block_x = start_x; block_x < end_x; block_x += stride) {
        blockIdx.x = block_x;
        blockIdx.y = block_y;
        blockIdx.z = block_z;
